...
FOLDS = {'1997': ('1997-05-01', '1998-04-01'),
         ...
         '2015full': ('2015-01-01', '2016-12-01'),
...
def main():
    ...
    parser.add_argument('--nmme-geoss2s', action='store_true', help='Previsões GEOSS2S emitidas em T-1')
    parser.add_argument('--heavy-rain-weight', type=float, default=1.0,
                        help='Peso de treino para alvos observados acima de 10 mm/dia; apenas pesquisa temporal')
    ...
    if args.heavy_rain_weight < 1:
        parser.error('--heavy-rain-weight deve ser >= 1')
    ...
    if args.nmme_geoss2s:
        from nmme_features import load_cache, add_features
        nmme_cache = load_cache()
    for exp in args.experiments:
        ...
        if args.heavy_rain_weight != 1:
            prefix = prefix.with_name(prefix.name + f'_heavy{args.heavy_rain_weight:g}')
        ...
        if args.ocean_indices:
            ...
            series = read_indices()
            train = add_indices(train, target_dates[tr], len(lat)*len(lon), series)
            val = add_indices(val, target_dates[va], len(lat)*len(lon), series)
        if args.nmme_geoss2s:
            train = add_features(train, target_dates[tr], lat, lon, nmme_cache)
            val = add_features(val, target_dates[va], lat, lon, nmme_cache)
        ...
        with threadpool_limits(limits=8):
            if model_name == 'LightGBM':
                observed_rain = train.target.to_numpy() + train.climatology.to_numpy()
                weights = np.where(observed_rain >= 10, args.heavy_rain_weight, 1.0).astype('float32')
                model = lgb.train(PARAMS,lgb.Dataset(train[features],label=train.target,weight=weights),num_boost_round=args.rounds)
        ...
        result = dict(experiment=exp,model=model_name,feature_group=group,features=features,
            fold=args.fold,stride=args.stride,train_years=args.train_years,atmosphere_history=args.atmosphere_history,rounds=args.rounds,heavy_rain_weight=args.heavy_rain_weight,rmse=score(pred,truth),climatology_rmse=score(baseline,truth),
