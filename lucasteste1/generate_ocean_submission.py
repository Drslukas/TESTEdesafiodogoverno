...
parser.add_argument('--nmme-geoss2s', action='store_true')
...
if args.nmme_geoss2s:
    frame = add_features(frame, dates[idx], lat, lon, nmme_cache)
...
model.save_model(str(OUT/('model_ocean_nmme_30years.txt' if args.nmme_geoss2s else 'model_ocean_30years.txt')))
del frame,arrays
gc.collect()
...
for i,date in enumerate(testdates):
    ...
    if args.nmme_geoss2s:
        f=add_features(f,pd.DatetimeIndex([date]),la,lo,nmme_cache)
...
suffix = '_nmme' if args.nmme_geoss2s else ''
result.to_csv(OUT/f'submission_ocean{suffix}_2023_2024.csv',index=False)
result[result.id.str.startswith('2023_')].to_csv(OUT/f'predictions_ocean{suffix}_2023.csv',index=False)
(OUT/f'submission_ocean{suffix}_metadata.json').write_text(json.dumps({'rows':len(result),'features':features,'train_end':str(dates[idx][-1]),'validation_report':'outputs/nmme_gate.json','warning':'Historical validation is not a Kaggle score; no automatic Kaggle upload.'},indent=2))
