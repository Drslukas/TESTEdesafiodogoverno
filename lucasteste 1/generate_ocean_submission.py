...
(OUT/'submission_ocean_metadata.json').write_text(json.dumps({'rows':len(result),'validation_2022_rmse':1.816201,'features':features,'train_end':str(dates[idx][-1]),'warning':'Retrospective monthly indices; release vintages not verified. Not a single December-issued annual forecast.'},indent=2))
